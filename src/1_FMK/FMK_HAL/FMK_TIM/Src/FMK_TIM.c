                                                ...
                                                t_sFMKTIM_PwmOpe f_PwmOpe_s)
{
    ...
    if(Ret_e == RC_OK)
    {
        ...
        if(Ret_e == RC_OK)
        {
            ...
            {
                //-------Activate channel-------------//
                if (timerInfo_ps->Channel_as[f_chnl_e].State_e == FMKTIM_CHNLST_DISACTIVATED)
                {
                    Ret_e = s_FMKTIM_Set_HwChannelState(f_timer_e, f_chnl_e, FMKTIM_CHNLST_ACTIVATED);
                }
            }
        }
    }
                                                    ...
                                                    t_uint16 *f_dutyCycle_pu16)
{
   ...
   || (f_channel_e >= FMKTIM_CHANNEL_NB))
    ...
    if(Ret_e == RC_OK)
    {
        ...
        if ((timerInfo_ps->isConfigured_b == (t_bool)False)
        ||  (timerInfo_ps->Channel_as[f_channel_e].IsChnlConfigure_b == (t_bool)False))
        {
            Ret_e = RC_ERROR_MISSING_CONFIG;
        }
        if(GETBIT(timerInfo_ps->Channel_as[f_channel_e].ErrState_u16, FMKTIM_ERRSTATE_OK != BIT_IS_SET_16B))
        {
            Ret_e = RC_ERROR_WRONG_STATE;
        }
        //---------get dutycycle only if channel is active---------//
        if (timerInfo_ps->Channel_as[f_channel_e].State_e == FMKTIM_CHNLST_DISACTIVATED)
        {
...
static void s_FMKTIM_BspRqst_InterruptMngmt(TIM_HandleTypeDef *f_timerIstce_ps, t_eFMKTIM_BspEvent f_cbEvnt_e)
{
    ...
    else
    {
        ...
        if(f_cbEvnt_e == FMKTIM_BSP_CB_PWM_PULSE_FINISHED)
        {
